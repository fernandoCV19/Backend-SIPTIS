...
public interface ProjectRepository extends JpaRepository<Project, Long> {
    ...
    @Query(value = "SELECT modality_.name_ AS modality, COUNT(projectId) AS number_projects FROM " +
            "( SELECT project_.id AS projectId, project_.modality_id AS modalityId " +
            ...
            " project_student.user_id_ =  siptis_user_career.siptis_user_id " +
            "AND siptis_user_career.career_id_ = :idCareer GROUP BY (project.id_) " +
            " ) result" +
            " RIGHT JOIN modality_ modality ON result.modalityId = modality.id " +
