...
public class TutorProjectReport {
    ...
    public static String generateReport(List<ProjectTutor> toReview, List<ProjectTutor> reviewed, List<ProjectTutor> accepted) {
        ...
        String fileName = "Report-tutor" + date + ".xlsx";
    ...
    private static void addBody(Sheet sheet, CellStyle contentStyle, CellStyle headerStyle, List<ProjectTutor> projects) {
        ...
        for (ProjectTutor projectTutor : projects) {
            String project = projectTutor.getProject().getName();
            String modality = projectTutor.getProject().getModality().getName();
            String teachers = getTeachers(projectTutor.getProject());
            String tutors = getTutors(projectTutor.getProject());
            String supervisors = getSupervisors(projectTutor.getProject());
            String tribunals = getTribunal(projectTutor.getProject());
    ...
    private static void addAccepted(Sheet sheet, CellStyle contentStyle, CellStyle headerStyle, List<ProjectTutor> accepted) {
        Row row = sheet.createRow(rowIndex);
    ...
    private static void addReviewed(Sheet sheet, CellStyle contentStyle, CellStyle headerStyle, List<ProjectTutor> reviewed) {
        Row row = sheet.createRow(rowIndex);
    ...
    private static void addToReview(Sheet sheet, CellStyle contentStyle, CellStyle headerStyle, List<ProjectTutor> toReview) {
        Row row = sheet.createRow(rowIndex);
