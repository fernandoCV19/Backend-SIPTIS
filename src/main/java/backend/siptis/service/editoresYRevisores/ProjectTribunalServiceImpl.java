...
import backend.siptis.model.pjo.dto.gestionProyecto.ProjectToTribunalHomePageDTO;
import backend.siptis.model.repository.editoresYRevisores.ProjectTribunalRepository;
...
public class ProjectTribunalServiceImpl implements ProjectTribunalService {
    ...
    @Override
    public ServiceAnswer getAllProjectsNotReviewedByIdTribunal(Long id) {
    ...
    @Override
    public ServiceAnswer getAllProjectsReviewedNotAcceptedByIdTribunal(Long id) {
        if(projectTribunalRepository.findById(id).isEmpty()) {
            return ServiceAnswer.builder().serviceMessage(ServiceMessage.ID_DOES_NOT_EXIST).data(null).build();
        }
    ...
    @Override
    public ServiceAnswer getAllProjectsAcceptedWithoutDefensePointsByIdTribunal(Long id) {
        if(projectTribunalRepository.findById(id).isEmpty()) {
            return ServiceAnswer.builder().serviceMessage(ServiceMessage.ID_DOES_NOT_EXIST).data(null).build();
        }
    ...
    @Override
    public ServiceAnswer getAllProjectsDefendedByIdTribunal(Long id) {
        if(projectTribunalRepository.findById(id).isEmpty()) {
            return ServiceAnswer.builder().serviceMessage(ServiceMessage.ID_DOES_NOT_EXIST).data(null).build();
        }
